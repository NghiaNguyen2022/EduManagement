...
$packageName = "qltruonghoc-vireon-$timestamp"
...
Copy-Item "deploy\ecosystem.config.cjs" (Join-Path $packageRoot "deploy")
Copy-Item "deploy\env.production.example" (Join-Path $packageRoot "deploy")
Copy-Item "deploy\README_VPS.md" (Join-Path $packageRoot "deploy")
Copy-Item "deploy\nginx" (Join-Path $packageRoot "deploy") -Recurse
Copy-Item "package.json" $packageRoot
Copy-Item "pnpm-lock.yaml" $packageRoot

"Thư mục chứa file người dùng tải lên. Không xóa thư mục này khi cập nhật." |
  Set-Content (Join-Path $packageRoot "uploads\README.txt") -Encoding UTF8

Compress-Archive -Path (Join-Path $packageRoot "*") -DestinationPath $zipPath

Write-Host ""
Write-Host "Đã tạo deploy package:"
Write-Host $zipPath
Write-Host ""
