 ...
 * Danh mục đơn vị, người dùng/vai trò. Không tạo dữ liệu nghiệp vụ (chương trình, hồ sơ giáo
...
export async function createDonViUnit(input: {
      ...
      // Hiện tại Danh mục đơn vị chỉ có một cấp: mọi trường/trung tâm/cơ sở nằm trực
      ...
      const created = await createDonVi({
            ...
            nguoiDaiDien: input.nguoiDaiDien?.trim() || null,
            maSoThue: input.maSoThue?.trim() || null,
            maGiayPhep: input.maGiayPhep?.trim() || null,
            giayPhepUrl: input.giayPhepUrl?.trim() || null,
      });
