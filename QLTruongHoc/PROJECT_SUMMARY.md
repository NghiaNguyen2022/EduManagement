...
## H08 — Hoàn phí / chuyển phí / bảo lưu (2026-07-23)

Khoảng trống tài chính lớn nhất còn lại (đã chốt ưu tiên sau khi đối chiếu 4 sản phẩm tham
khảo — xem mục "Đối chiếu tham khảo ngoài" phía trên). Hoàn tất toàn bộ mục H (H01-H09).

- **Bảng mới** `DieuChinhKhoanPhaiThu` (`database/023_add_dieu_chinh_khoan_phai_thu.sql`,
  `drizzle/schemas/taiChinh.ts`) — yêu cầu điều chỉnh gắn với một `KhoanPhaiThu`, trạng thái
  `cho_duyet/da_duyet/tu_choi`. **Chỉ là yêu cầu khi tạo — chưa đổi số liệu tài chính**, đúng
  BPD 7.6 "Hoàn/hủy/điều chỉnh theo quy trình phê duyệt" (khác Miễn giảm/Thu tiền ở H04/H05,
  vốn có hiệu lực ngay).
- **Quyền mới** `tai_chinh.duyet` (`seedAuthFoundation.ts`, đã seed vào DB dev) — tách khỏi
  `tai_chinh.quan_ly` (người lập yêu cầu). Chưa gán sẵn cho vai trò nào ngoài
  `quan_tri_he_thong` (được mọi quyền theo cơ chế sẵn có) — cần gán tay qua `/roles` cho vai
  trò phù hợp khi dùng thật.
- **Duyệt bắt buộc khác người lập**: `duyetDieuChinh` chặn cứng nếu `nguoiDuyetId === nguoiTaoId`,
  không chỉ dựa vào mã quyền khác nhau (một tài khoản có thể có cả hai quyền cùng lúc).
- **Hiệu lực khi duyệt**: hoàn phí trừ `daThu` khoản nguồn (không vượt số đã thu thật); chuyển
  phí trừ nguồn + cộng đích (không vượt "còn phải thu" của đích); bảo lưu chỉ ghi nhận quyết
  định, chưa đổi `KhoanPhaiThu` (bảo lưu ở mức tổng thể học sinh đã có từ D06 — xem giới hạn
  trong `docs/analysis/H08_hoan_phi_chuyen_phi_bao_luu.md` mục 4). Tái dùng đúng
  `updateKhoanPhaiThuDaThu`/`tinhTrangThaiKhoanPhaiThu` đã có từ H05, không viết lại.
- **UI**: gộp trong `KyThuDetailPage.tsx` — nút "Điều chỉnh" cạnh Thu tiền/Miễn giảm/Lịch sử
  thu (không tách trang duyệt riêng, theo đúng phạm vi đã chốt trước khi code). Form ẩn/hiện
  số tiền và khoản đích theo đúng loại điều chỉnh; bảng lịch sử điều chỉnh hiện cột "Thao tác"
  (Duyệt/Từ chối) chỉ khi tài khoản có `tai_chinh.duyet`.
- **Test tay qua UI thật** với `demo_ketoan` (có `tai_chinh.quan_ly`, cố tình không có
  `tai_chinh.duyet`, phải đổi mật khẩu tạm — mật khẩu mới `Edu@123Qaz2`):
  - Tạo yêu cầu bảo lưu cho học sinh Phạm Gia Hân (TTNN-Q8) — thành công, hiện đúng "Chờ
    duyệt", không thấy cột "Thao tác" (đúng phân quyền) — PASS.
  - Hoàn phí khi `daThu = 0` — nút "Gửi yêu cầu" tự khoá tới khi nhập số tiền hợp lệ — PASS.
  - **Phát hiện và sửa luôn 1 lỗi có sẵn từ trước** (không thuộc H08): `KyThuDetailPage.tsx`
    gọi `listLopHocApi()` không bọc lỗi trong `Promise.all` khi tải trang — vai trò kế toán
    không có `lop_hoc.xem` nên cả trang chi tiết kỳ thu vỡ hoàn toàn (403) dù có đủ quyền tài
    chính, chỉ vì phần "Sinh khoản phải thu" cần danh sách lớp. Sửa bằng
    `listLopHocApi().catch(() => [])` — không chặn phần còn lại của trang.
  - **Chưa test được**: luồng duyệt/từ chối thật (cần tài khoản thứ hai có `tai_chinh.duyet`)
    và hoàn phí/chuyển phí với số tiền thật > 0 (công cụ tự động hoá trình duyệt gặp khó khi
    nhập `CurrencyInput` trong phiên test này, đã xác nhận validate đúng qua trường hợp số tiền
    vượt mức khi thử nghiệm ở form Thu tiền — cùng cơ chế). Đã review kỹ qua code +
    `pnpm typecheck`.
- Checklist: H08 đã tick trong `docs/00_MASTER_CHECKLIST.md`, mục H hoàn tất toàn bộ.
- Kiểm tra: `pnpm typecheck`, `pnpm build` — PASS.
